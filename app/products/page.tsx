    ...
    <LayoutWrapper HeaderStyle="two">
      ...
      <main className="shop-page">
        <div className="auto-container">
          ...
          <section className="product-results" aria-label="Product results"><div className="results-heading"><span>Showing <b>{filteredProducts.length}</b> of {products.length} products</span></div><div className="product-grid">{filteredProducts.map((product) => <article className="product-card" key={product.id}><Link className="product-card-link" href={`/products/${product.id}`}><div className="product-image">{product.badge && <span className="product-badge">{product.badge}</span>}<Image src={product.image} alt={product.name} width={540} height={400} /></div><div className="product-card-content"><span className="product-category">{product.category}</span><h3>{product.name}</h3><div className="product-rating"><span>{"★".repeat(Math.round(product.rating))}</span> <small>{product.rating}</small></div></div></Link><div className="product-card-action"><EnquiryModal productName={product.name} /></div></article>)}</div></section>
        </div>
      </main>
    </LayoutWrapper>
  );
